...
class PlayState extends MusicBeatState
{
	...
	public var spawnTime:Float = 1600;
	...
	override public function create()
	{
		...
		if (ClientPrefs.data.timeBarStyle != 'Dave Engine') uiGroup.add(timeBar);
		
		switch (ClientPrefs.data.timeBarStyle) {
			case 'VS Impostor':
				timeBarBG.loadGraphic(Paths.image('timeBars/timeBarImpostor'));
			...
			case 'Dave Engine':
				...
				timeBarBG = new AttachedSprite('timeBars/timeBarDave&Bambi');
				timeBarBG.screenCenter(X);
				timeBarBG.y = timeTxt.y + (timeTxt.height / 4);
				...
				insert(members.indexOf(uiGroup), timeBar);

			case 'Doki Doki+':
				timeBarBG.loadGraphic(Paths.image("timeBars/timeBarDokiDoki+"));
				timeBarBG.screenCenter(X);
...
private function popUpScore(note:Note = null, ?miss:Bool = false):Void
{
	...
	if (showRating && ClientPrefs.data.ratingPopups) {
	...
	if (showCombo && ClientPrefs.data.comboPopups && combo >= 10) {
		comboSpr = popUpGroup.recycle(Popup);
		comboSpr.setupCombo(uiPrefix + 'combo' + uiPostfix);
		comboSpr.alphaTween();
		popUpGroup.insert(0, comboSpr);
	}

	if (showComboNum && ClientPrefs.data.comboNumsPopups)
	{
	...
	if (ClientPrefs.data.ratingPopups) popUpGroup.sort((o, a, b) ->
	{
